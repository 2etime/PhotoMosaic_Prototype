...
class Mouse {
    ...
    public static func GetMouseViewportPosition()->float2{
        let x = (overallMousePosition.x - MainView.ScreenSize.x * 0.5) / (MainView.ScreenSize.x * 0.5)
        let y = (overallMousePosition.y - MainView.ScreenSize.y * 0.5) / (MainView.ScreenSize.y * 0.5)
        return float2(x, y)
    }
}
